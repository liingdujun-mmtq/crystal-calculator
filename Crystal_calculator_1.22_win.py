...
def cal_d(h,k,l):
    ...
    V=a*b*c*math.sqrt(1-(cos(alpha))**2-(cos(beta))**2-(cos(gamma))**2+2*cos(alpha)*cos(beta)*cos(gamma))
        ...
        if a==b and a==c:
            ...
            for h in range(5):
                for k in range(5):
                    for l in range(5):
                        if h!=0 or k!=0 or l!=0:
                            d=math.sqrt(1/(h**2/a**2+k**2/b**2+l**2/c**2))
                            message.append([h,k,l,d,1/d])
        ...
        #四方        
            ...
            for h in range(5):
                for k in range(h+1):
                    for l in range(5):
                        if h!=0 or k!=0 or l!=0:
                            d=a/math.sqrt(h**2+k**2+l**2/(c/a)**2)
                            message.append([h,k,l,d,1/d])
        elif b==c:
            for h in range(5):
                for k in range(5):
                    for l in range(k+1):
                        if h!=0 or k!=0 or l!=0:
                            d=b/math.sqrt(h**2/(a/b)**2+k**2+l**2)
                            message.append([h,k,l,d,1/d])
        elif a==c:
            for h in range(5):
                for k in range(5):
                    for l in range(h+1):
                        if h!=0 or k!=0 or l!=0:
                            d=a/math.sqrt(h**2+k**2/(b/a)**2+l**2)
                            message.append([h,k,l,d,1/d])
    else:
        for h in range(-4,5):
            for k in range(-4,5):
